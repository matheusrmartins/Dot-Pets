...
import 'package:xlo/blocks/login/button_state.dart';
import 'package:xlo/blocks/login/field_state.dart';
import 'package:xlo/blocks/login/login_bloc_state.dart';
...
class LoginBloc with LoginValidator {
  ...
  Stream<FieldState> get outEmail => Rx.combineLatest2(_emailController.stream.transform(emailValidator), outState,
          (a, b) {
            a.enabled = b.state != LoginState.LOADING;
            return a;
          });

  Stream<FieldState> get outPassword =>Rx.combineLatest2(_passwordController.stream.transform(passwordValidator), outState,
          (a, b) {
        a.enabled = b.state != LoginState.LOADING;
        return a;
      });
  Stream<ButtonState> get outLoginButton =>
      Rx.combineLatest3(outEmail, outPassword, outState,
              (a, b, c) {
          return ButtonState(
